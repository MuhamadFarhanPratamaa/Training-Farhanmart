...
class Penjualan(models.Model):
    ...
    detailpenjualan_ids = fields.One2many(
        comodel_name='farhanmart.detailpenjualan',
    ...
    def _compute_totalbayar(self):
        for rec in self:
            a = sum(self.env['farhanmart.detailpenjualan'].search([('penjualan_id', '=', rec.id)]).mapped('subtotal'))
            rec.total_bayar = a
    ...
    #         a=[]
    #         for rec in self:
    #             a = self.env['wikumart.detailpenjualan'].search([('penjualan_id','=',rec.id)])
    #             print(a)
    #         for ob in a:
    ...
    def unlink(self):
        if self.filtered(lambda line: line.state != 'draft'):
            ...
            if self.detailpenjualan_ids:
                a = []
                for rec in self:
                    a = self.env['farhanmart.detailpenjualan'].search([('penjualan_id', '=', rec.id)])
                    print(a)
                for ob in a:
                    print(str(ob.barang_id.name) + ' ' + str(ob.qty))
                    ob.barang_id.stok += ob.qty
    ...
    def write(self, vals):
        ...
        for rec in self:
            b = self.env['farhanmart.detailpenjualan'].search([('penjualan_id', '=', rec.id)])
            print(a)
            print(b)
            for databaru in b:
                if databaru in a:
...
class DetailPenjualan(models.Model):
    _name = 'farhanmart.detailpenjualan'
    _description = 'New Description'

    name = fields.Char(string='Nama')
    penjualan_id = fields.Many2one(comodel_name='farhanmart.penjualan', string='Detail Penjualan')
    barang_id = fields.Many2one(comodel_name='farhanmart.barang', string='List Barang')
    ...
    def create(self, vals):
        ...
        if record.qty:
            self.env['farhanmart.barang'].search([('id', '=', record.barang_id.id)]).write(
